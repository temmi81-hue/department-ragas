...
try {
  ...
  for (const chapter of scenario.chapters ?? []) {
    await page.locator('#demo-card').evaluate((e,c)=>{e.innerHTML=`${c.title ?? ''}<small>${c.subtitle ?? ''}</small>`;e.style.opacity='1';},{title:chapter.title,subtitle:chapter.subtitle}); await sleep(chapter.title ? 1100 : 400); await page.locator('#demo-card').evaluate(e=>e.style.opacity='0');
    for (const step of chapter.steps ?? []) { const [kind, value] = Object.entries(step)[0]; const cfg = typeof value === 'object' ? value : { duration:value }; if (step.caption && !cfg.caption) cfg.caption = step.caption;
      // Caption is shown for the duration of the step's own action (typing/click/highlight/etc.)
      // instead of a fixed flash before it, so it stays legible while something is actually
      // happening on screen rather than vanishing right before the action starts.
      if (cfg.caption) await page.locator('#demo-caption').evaluate((e, t) => { e.textContent = t; e.style.opacity = '1'; }, cfg.caption);
      if (kind === 'wait') await sleep(Number(value));
      if (kind === 'mark') marks[typeof value === 'string' ? value : cfg.name] = Date.now() - recordingStart;
      if (kind === 'scroll') { if (cfg.align === 'center' || cfg.align === 'start') await page.locator(cfg.selector).evaluate((el, block) => el.scrollIntoView({ block }), cfg.align); else await page.locator(cfg.selector).scrollIntoViewIfNeeded(); await sleep(cfg.duration ?? 1000); }
      ...
      if (cfg.caption) { await sleep(300); await page.locator('#demo-caption').evaluate(e => e.style.opacity = '0'); }
    }
  }
